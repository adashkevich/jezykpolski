...
kobieta|NOUN::vocab:ru-pl-choice   женщина → kobieta (выбор среди вариантов)
kobieta|NOUN::vocab:ru-pl-input    женщина → kobieta (написать по-польски)
...
```text
kobieta|NOUN
  Значение         64%   ← среднее по vocab:pl-ru, vocab:ru-pl-choice, vocab:ru-pl-input
...
learning  есть записи, но vocabMaturity < 0.35
          ИЛИ слово ни разу не набрано успешно (см. ниже)
known     vocabMaturity ≥ 0.35 И слово набрано хотя бы раз
mastered  vocabMaturity ≥ 0.9 И morphologyMaturity ≥ 0.9 (для слов с парадигмой) И слово набрано хотя бы раз
```

**Гейт по этапу изучения (задача 28, ужесточён задачей 37, FR-83).** У перевода три этапа, и
все выражены навыками: `vocab:pl-ru` — узнавание по-русски (выбор перевода из списка),
`vocab:ru-pl-choice` — узнавание по-польски (выбор слова среди вариантов),
`vocab:ru-pl-input` — воспроизведение (написать слово по-польски). Каждый следующий этап
лениво материализуется только после того, как предыдущий набрал стабильность выше своего
порога (`learning/progress/stage.ts#shouldUnlockCuedRecall` / `shouldUnlockProduction`,
7 и 10 дней соответственно) — задача 28 держала это правило на состоянии FSRS (`review` после
одного верного ответа), задача 37 явно усложнила его до нескольких повторений.

Статус `known`/`mastered` требует не просто существования `vocab:ru-pl-input`, а того, чтобы
он хоть раз выпустился в `review` (`stage.ts#hasGraduatedProduction`,
`aggregate.ts#WordAggregate.productionGraduated`) — то есть слово реально набрано успешно хотя
бы раз. Пока это не так, слово остаётся `learning`, каким бы зрелым ни было узнавание. Само
усреднение `vocabMaturity` по трём навыкам этого не гарантирует — достаточно зрелые
`vocab:pl-ru`/`vocab:ru-pl-choice` вытягивают среднее выше порога и без единого набранного
ответа.
...
**Побуквенный ввод (`input`/`form-input`, задача 29, FR-84/FR-85/FR-86).** Эти два типа
проверяются не одной строкой, а живьём — `LetterSlotsInput` (`features/session-runner/**`)
опирается на чистую машину состояний `learning/exercises/letter-attempt.ts` и передаёт её
итог (`{ mistakes, hintsUsed, revealed, letterCount }`) третьим, структурно самостоятельным
вариантом `ExerciseGradeResult` в `policy.ts#mapResultToRating` — маппинг для него:

```text
без ошибок и подсказок                          → Easy
хотя бы одна ошибка или подсказка               → Hard
слово раскрыто «глазком», либо все буквы открыты
подсказками                                     → Again
```

`Hard`, а не `Again`, за единственную ошибку/подсказку — иначе написание стало бы
непроходимым: одна описка в диакритике не должна сбрасывать навык в relearning с нуля. `GradeResult.correct`
при этом не переопределяется — оно по-прежнему значит «финальная строка совпала с принятым
ответом»; рейтинг — отдельный канал, по которому «согласился слово выучить» отличимо от
«вспомнил не без труда».
...
1. **Повтор ошибки внутри той же сессии.** Пользователь только что увидел правильный ответ.
   ...
   **Изменено задачей 45**: в «точность» (главная, итог сессии) повтор не входит — считаются только
   первые ответы на навык в сессии и только чистые (`clean`/`firstInSession`, `accuracyAttempts`/
   `accuracyClean` в `dailyStats`); счётчики `reviewsCount`/`correctCount` по-прежнему считают все ответы.
...
Контракт компонента (`ExerciseProps<E>`) не поменялся составом полей задачей 29, но
`onAnswer` получил необязательный второй аргумент: `onAnswer(answer: string, attempt?:
TypedAttemptOutcome): void`. Его передают только `InputExercise`/`FormInputExercise` (через
общий `LetterSlotsInput`) — итог побуквенной попытки, который `SessionRunner` прокидывает в
`submitAnswer` для расчёта рейтинга (§6.2, §7.3). Остальные компоненты аргумент просто не
передают.

### 7.2 Выбор типа упражнения (`picker.ts`)

Правил два, и они устроены по-разному.

**Перевод — тип задаётся измерением навыка, а не состоянием FSRS** (FR-80, задача 28, третий
этап добавлен задачей 37):

```text
vocab:pl-ru         →  choice   (этап 1: узнавание по-русски, выбрать перевод из списка)
vocab:ru-pl-choice  →  choice   (этап 2: узнавание по-польски, выбрать слово среди вариантов)
vocab:ru-pl-input   →  input    (этап 3: написать слово по-польски)
                                (или self-assess в состоянии review — при явной настройке)
```

`PL→RU input` («напечатать русский перевод») в прогрессии не участвует — FR-51/FR-52 отменены.
`PickerOptions.forceCategory` (`picker.ts`'s собственное ограничение «Тип задания») на перевод
по-прежнему не влияет: у каждого этапа тип фиксирован его измерением, выбирать там нечего.
Задача 37 добавила ДРУГОЙ механизм «Тип задания» специально для перевода — он не меняет тип
уже выбранного упражнения, а исключает навыки нежелательного этапа из выборки целиком
(`session-scope.ts#filterForVocabExerciseType` для ежедневной сессии,
`build-practice-queue.ts#vocabMatchesExerciseType` для Practice).

**Морфология — по состоянию навыка, как и раньше:**

```text
skill.state === 'new'                                 → form-choice
skill.state === 'learning'  и reps < 2                → form-choice
skill.state === 'learning'  и reps ≥ 2                → form-input
skill.state === 'review'                              → form-input (или self-assess при настройке)
skill.state === 'relearning'                          → form-choice
```

Разнесение этапов по времени (FR-81) достигается не сценарием внутри `picker.ts`, а
материализацией: `vocab:ru-pl-choice`/`vocab:ru-pl-input` вообще не существуют как
`SkillRecord`, пока `shouldUnlockCuedRecall`/`shouldUnlockProduction` (§5.4) не разрешат их
создать, а очередь текущей сессии строится целиком заранее — значит, самое раннее, где может
появиться следующий этап, это следующая сессия.
...
- **польские диакритики не нормализуются** при вводе польского: `zolty ≠ żółty` — это часть навыка.
  Но ответ без диакритики распознаётся как «почти верно» → рейтинг `Hard`;
...
**Побуквенная подсветка постфактум (`answer-diff.ts`, FR-58, задача 28).** Сам `grade`
подсветку не считает — он только сообщает, с каким из принятых вариантов сравнивать
(`closest`, ближайший по расстоянию Левенштейна). Разметку строит отдельная чистая функция
`diffAnswer(введённое, эталон)`:

- сравнение посимвольное, без учёта регистра, с `ё → е`; диакритики **не** сворачиваются —
  `o` против `ó` это ошибка, красная буква;
- выравнивание — Левенштейн с восстановлением пути, а не сравнение «i-й символ с i-м»: иначе
  одна пропущенная буква красит весь хвост слова и подсветка перестаёт показывать, где ошибка;
- результат — две размеченные строки: введённая (`match` / `wrong` / `extra`) и эталонная
  (`match` / `wrong` / `missing`).

Рендерер — `components/app/AnswerDiff.tsx` (совпавшая буква зелёная, ошибочная красная;
NFR-11 обеспечивается подчёркиванием/зачёркиванием и `sr-only`-строкой с обычным текстом
рядом с раскрашенным рядом букв). С задачи 29 у него два потребителя, оба постфактум, после
отправки ответа целиком: ячейки таблиц склонения/спряжения (`ExpectedDiffLine`) и список
ошибок на экране результатов (`DiffText`). Панель `ExerciseFeedback` больше не рендерит
диффом свой собственный «Ты написал / Правильно» — эта пара строк (`AnswerDiffLines`)
удалена вместе с последним потребителем.

**Побуквенная подсветка вживую (`learning/exercises/letter-attempt.ts`, FR-59/FR-84/FR-85/
FR-86, задача 29).** Для `input`/`form-input` эталон и `diffAnswer` не участвуют в самом
процессе набора — вместо одной строки на выходе есть чистая машина состояний,
проверяющая **каждую** нажатую букву сразу, посимвольно, той же нормализацией, что и
`answer-diff.ts` (регистр + `ё → е`; диакритики не сворачиваются). Слоты строятся по
`accepted[0]`, кандидаты сужаются по мере ввода среди вариантов **равной длины** (несколько
допустимых написаний формы, например `aborcji`/`aborcyj`, не должны требовать заранее
знать, сколько слотов рисовать). Неверная буква не двигает курсор — следующее нажатие её
заменяет; ошибка считается один раз на ячейку, а не на нажатие. Итог попытки
(`{ mistakes, hintsUsed, revealed, letterCount }`) уходит в `policy.ts#mapResultToRating`
(§6.2) — рендерит его `features/session-runner/components/LetterSlotsInput.tsx`, общий для
`InputExercise` и `FormInputExercise`.
...
interface ReviewLogRecord {
  ...
  // задача 45 — необязательные, без миграции Dexie; у старых логов их нет (`accuracy.ts` даёт запасное правило)
  clean?: boolean;          // без ошибок, подсказок и «глазка» (`correct` — лишь «в итоге совпало»)
  firstInSession?: boolean; // первый ответ на навык в сессии — только такие идут в точность
  assist?: 'hinted' | 'corrected'; // чем не безупречен верный набор (для итога сессии)
}
```
...
**`version(2)` (задача 37)** — первая реальная миграция в проекте: переименовывает старое
измерение `vocab:ru-pl` в `vocab:ru-pl-input` и досоздаёт `vocab:ru-pl-choice` копией того же
SRS-состояния (`db/legacy-vocab-migration.ts`), в `skills` и в `reviewLogs.skillId`. Индексные
строки не меняются — `dimension` никогда не индексировался, `skillId` остаётся первичным
ключом, поэтому вся работа идёт в `.upgrade()`, а не в новом `.stores()`.

`wordProgress` эта миграция НЕ трогает: `computeWordProgress` требует загруженного индекса
контента (`getIndexStore()`/`getParadigm()`), а `db.open()` выполняется до того, как
`ContentProvider` вообще смонтирован. Пересчёт `wordProgress` под новой моделью — отдельный
одноразовый проход `recomputeAll()` в `StartupMigrations.tsx` (тот же приём, что задача 28
уже ввела для своего собственного гейта по этапу — второй `meta.repository.ts#runOnce`-ключ,
а не правка первого).

Тот же переименовывающий переход переиспользуется при импорте бэкапа
(`db/repositories/backup.repository.ts#prepareImport`) — файл, экспортированный до задачи 37,
всё ещё содержит `vocab:ru-pl` и должен нормализоваться перед записью, иначе реимпортировал бы
устаревшее измерение в уже мигрированную базу. `CURRENT_BACKUP_SCHEMA_VERSION` при этом не
бампается — формат файла не изменился, изменилось только значение одной строки.
