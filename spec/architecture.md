...
learning  есть записи, но vocabMaturity < 0.35
          ИЛИ этап 2 ещё не открыт (см. ниже)
known     vocabMaturity ≥ 0.35 И этап 2 открыт
mastered  vocabMaturity ≥ 0.9 И morphologyMaturity ≥ 0.9 (для слов с парадигмой) И этап 2 открыт
```

**Гейт по этапу изучения (задача 28, FR-83).** У перевода два этапа, и оба выражены навыками:
`vocab:pl-ru` — узнавание (выбор значения из списка), `vocab:ru-pl` — воспроизведение
(написать слово по-польски). Второй лениво материализуется только после того, как первый
выпустился в `review` (`learning/progress/stage.ts#shouldUnlockProduction`). Пока записи
`vocab:ru-pl` нет, слово остаётся `learning`: узнавание — часть владения словом, но не всё.
Само усреднение `vocabMaturity` этого не гарантирует — достаточно зрелый `vocab:pl-ru` в
одиночку вытягивает среднее выше порога.
...
`Hard`, а не `Again`, за единственную ошибку/подсказку — иначе этап 2 стал бы непроходимым:
одна описка в диакритике не должна сбрасывать навык в relearning с нуля. `GradeResult.correct`
...
**Перевод — тип задаётся направлением навыка, а не состоянием FSRS** (FR-80, задача 28):

```text
vocab:pl-ru  →  choice   (этап 1: узнавание, выбрать значение из списка)
vocab:ru-pl  →  input    (этап 2: написать слово по-польски)
                         (или self-assess в состоянии review — при явной настройке)
```

`PL→RU input` («напечатать русский перевод») и `RU→PL choice` в прогрессии не участвуют —
FR-51/FR-52 отменены. Ограничение «Тип задания» (`forceCategory`) на перевод не влияет:
выбирать там нечего, направление уже всё определило.
...
материализацией: `vocab:ru-pl` вообще не существует как `SkillRecord`, пока
`shouldUnlockProduction` (§5.4) не разрешит его создать, а очередь текущей сессии строится
целиком заранее — значит, самое раннее, где может появиться этап 2, это следующая сессия.
