...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        // DESIGN.md surface level 1: white, 1px micro-border, ambient low shadow, 20px radius.
        'group/card flex flex-col gap-(--card-spacing) overflow-hidden rounded-2xl border border-border bg-card py-(--card-spacing) text-body-md text-card-foreground shadow-card [--card-spacing:--spacing(5)] has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:[--card-spacing:--spacing(4)] data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-t-2xl *:[img:last-child]:rounded-b-2xl',
        className,
      )}
      {...props}
    />
...
function CardTitle({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        'font-heading text-headline-sm text-foreground group-data-[size=sm]/card:text-label-lg',
        className,
      )}
      {...props}
    />
